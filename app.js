...
async function loadRecords(){
  ...
  if(r&&typeof r==='object'){
    ...
    if(r.settings&&r.settings.status_items&&Array.isArray(r.settings.status_items)){
      ...
      if(items.length>0){
        S_OPTS=items.map(i=>i.label);
        // [NEW] 色設定も反映（カスタム項目含む）
        items.forEach(i=>{if(i.color)S_COL[i.label]=i.color;});
      }
    }
  }
  renderMap();
...
function openStatusItemsEditor(modal, box) {
  ...
  let currentItems = S_OPTS.filter(s => s !== '確認のみ').map(s => ({ label: s, enabled: true, color: S_COL[s]||'#3498db' }));
