...
function initFilters(){
  ...
  // 個人設定保存後の再生成時に選択状態を復元
  selBlocks.forEach(c=>{const el=document.getElementById('bfc-'+c);if(el)el.classList.add('on');});
  selCrops.forEach(k=>{
    const el=document.getElementById('cgfc-'+k)||document.getElementById('cgfc-other-'+k.replace(/\s+/g,'_').replace(/[^\w\u3040-\u9fff]/g,'X'));
